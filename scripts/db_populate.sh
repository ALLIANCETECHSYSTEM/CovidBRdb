...
sed '1,2d' $CSV_DIR/covidbr-regions.csv | csvsql -H --query 'SELECT strftime("%Y-%m-%d", (`data`)), `regiao`, `estado`, `casosNovos`, `casosAcumulados`, `obitosNovos`, `obitosAcumulados` FROM covid_regions' | sed '1d' >$CSV_DIR/new_covidbr-regions.csv
